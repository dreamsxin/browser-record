...
export const SnapshotTabsView: React.FunctionComponent<{
  ...
  const screencastUrl = React.useMemo(() => {
    ...
    for (const candidate of page.screencastFrames) {
      if (candidate.timestamp > playback.currentTime)
        break;
      frame = candidate;
    }
  ...
  return <div className='snapshot-tab vbox'>
    ...
    {screencastUrl ? <div className='snapshot-wrapper'><img src={screencastUrl} style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} /></div> : <SnapshotView
