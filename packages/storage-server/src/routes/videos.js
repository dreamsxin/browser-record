...
function buildVideoRouter(config) {
  ...
  router.post('/upload', requireUploadToken(config), upload.single('archive'), async (req, res, next) => {
    try {
      ...
      const now = Date.now();
      getDb().prepare(`INSERT INTO video_archives(id,employee_id,session_id,archive_path,manifest_path,file_size,upload_time,status)
        VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(employee_id,session_id) DO UPDATE SET archive_path=excluded.archive_path,manifest_path=excluded.manifest_path,file_size=excluded.file_size,upload_time=excluded.upload_time,status=excluded.status`)
        .run(randomUUID(), employeeId, String(sessionId), path.relative(root, archivePath), path.relative(root, manifestPath), fs.statSync(archivePath).size, now, status || 'closed');
      res.json({ ok: true });
    } catch (err) { next(err); }
  });
