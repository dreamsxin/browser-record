...
const PartitionedWorkbench: React.FunctionComponent<WorkbenchProps & { partition: string }> = props => {
  ...
  const [selectedPageId, setSelectedPageId] = usePartitionedState<string | undefined>('selectedPageId');
  ...
  const { boundaries } = React.useMemo(() => {
    ...
    if (boundaries.maximum <= boundaries.minimum)
      boundaries.maximum = boundaries.minimum + 1000;
    // Leave some nice free space on the right hand side.
    boundaries.maximum += (boundaries.maximum - boundaries.minimum) / 20;
    return { boundaries };
  }, [model]);

  const framePages = React.useMemo(() => model?.pages.filter(page => page.screencastFrames.length) || [], [model]);
  React.useEffect(() => {
    if (!framePages.some(page => page.pageId === selectedPageId))
      setSelectedPageId(framePages[0]?.pageId);
  }, [framePages, selectedPageId, setSelectedPageId]);

  const hasScreencastFrames = framePages.length > 0;
  const playback = usePlayback(actions || [], selectedAction, onActionSelected, selectedTime, boundaries, hasScreencastFrames);
    ...
    <SplitView
      ...
      main={<SplitView
        ...
        main={<SnapshotTabsView
          ...
          onPageSelected={setSelectedPageId} />}
        sidebar={
          <TabbedPane
