...
export const FilmStrip: React.FunctionComponent<{
  ...
  const pagesWithFrames = model?.pages.filter(page => page.screencastFrames.length) || [];
  const screencastFrames = pagesWithFrames[pageIndex]?.screencastFrames;
  let previewImage = undefined;
  let previewSize = undefined;
  if (previewPoint !== undefined && screencastFrames && screencastFrames.length && measure.width > 0) {
    const previewTime = boundaries.minimum + (boundaries.maximum - boundaries.minimum) * previewPoint.x / measure.width;
    const previewIndex = Math.max(0, upperBound(screencastFrames, previewTime, timeComparator) - 1);
    previewImage = screencastFrames[previewIndex];
    const fitInto = {
  ...
  return <div className='film-strip' ref={ref}>
    <div className='film-strip-lanes' ref={lanesRef}>{
      pagesWithFrames.map((page, index) => <FilmStripLane
        boundaries={boundaries}
        page={page}
        width={measure.width}
        key={index}
...
const FilmStripLane: React.FunctionComponent<{
  ...
  const boundariesDuration = Math.max(1, boundaries.maximum - boundaries.minimum);
  ...
  const effectiveWidth = Math.max(0, (endTime - startTime) / boundariesDuration * width);
  const frameCount = Math.max(1, (effectiveWidth / Math.max(1, frameSize.width + 2 * frameMargin)) | 0);
  const frameDuration = frameCount > 1 ? (endTime - startTime) / frameCount : 0;

  const frames: React.JSX.Element[] = [];
  if (frameCount > 1 && Number.isFinite(frameDuration)) {
    for (let i = 0; i < frameCount; ++i) {
      const time = startTime + frameDuration * i;
      const index = Math.max(0, upperBound(screencastFrames, time, timeComparator) - 1);
