...
// Browser Record patch: trace append buffers are flushed on a short timer so
// live=true tracing does not retain a whole session in memory. The archive
// format remains unchanged; only the write timing changes.
const APPEND_FLUSH_INTERVAL_MS = Number(process.env.PLAYWRIGHT_TRACE_FLUSH_INTERVAL_MS || 1000);
...
export class SerializedFS {
  ...
  private _appendFlushTimer: NodeJS.Timeout | undefined;
  ...
  appendFile(file: string, text: string, flush?: boolean) {
    ...
    if (flush || size >= APPEND_CHUNK_SIZE)
      ...
      this._scheduleAppendFlush();
  }

  private _scheduleAppendFlush() {
    if (this._appendFlushTimer)
      return;
    this._appendFlushTimer = setTimeout(() => {
      this._appendFlushTimer = undefined;
      for (const file of this._buffers.keys())
        this._flushFile(file);
    }, APPEND_FLUSH_INTERVAL_MS);
    this._appendFlushTimer.unref?.();
  }
